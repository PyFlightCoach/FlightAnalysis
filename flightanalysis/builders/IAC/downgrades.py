...
dgs = DownGrades([
    ...
    dg("roll_rate", "roll_rate", measures.roll_rate(), sms.rollrate_lowpass(order=5), None, IAC.intra.rollrate),
    dg("roll_smoothness", "roll_smoothness", measures.abs_roll_rate(), sms.lowpass(cutoff=2, order=5), None, IAC.intra.rollsmoothness),
    ...
    dg("rolling_loop_roll_angle", "roll", measures.roll_angle_p(), None, sels.last(), IAC.intra.roll),

    dg("stallturn_width", "width", measures.stallturn_width(), None, None, IAC.intra.stallturn_width),
    dg("stallturn_speed", "speed", measures.vertical_speed(), None, sels.first_and_last(), IAC.intra.stallturn_speed),
    ...
    dg("recovery_rate_delta", "recovery", measures.delta_p(), None, sels.autorot_recovery(rot=np.pi/24), IAC.intra.recovery_roll_rate ),#roll rate reducing in last 45 degrees of turn
    ...
    dg("pitch_after_slowdown", "pitch", measures.pitch_attitude(), None, sels.after_slowdown(sp=13), IAC.intra.track),
    dg("yaw_after_slowdown", "yaw", measures.yaw_attitude(), None, sels.after_slowdown(sp=13), IAC.intra.track),

    dg("track_y_after_speedup", "track_y", measures.track_y(), sms.lowpass(cutoff=4, order=5), sels.after_speedup(sp=13), IAC.intra.track),
    dg("track_z_after_speedup", "track_z", measures.track_z(), sms.lowpass(cutoff=4, order=5), sels.after_speedup(sp=13), IAC.intra.track),
    dg("initial_track_y_after_speedup", "itrack_y", measures.track_y(), sms.lowpass(cutoff=4, order=5), [sels.after_speedup(sp=13), sels.first()], IAC.intra.end_track),
    dg("initial_track_z_after_speedup", "itrack_z", measures.track_z(), sms.lowpass(cutoff=4, order=5), [sels.after_speedup(sp=13), sels.first()], IAC.intra.end_track),

    dg("pitch_before_speedup", "pitch", measures.pitch_attitude(), None, sels.first(), IAC.intra.end_track),

    dg("end_yaw", "yaw", measures.yaw_attitude(), None, sels.last(), IAC.intra.end_track),
])
