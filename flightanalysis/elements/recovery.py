...
import numpy as np
from geometry import Transformation, PY
...
class Recovery(Element):
    ...
    def create_template(self, istate: State, fl: State = None) -> State:
        return (
            Line("recovery", self.speed, self.length, 0)
            .create_template(istate, fl)
            .superimpose_rotation(PY(), -np.arctan2(istate.vel.z, istate.vel.x)[-1])
