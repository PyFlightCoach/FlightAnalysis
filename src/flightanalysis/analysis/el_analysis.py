...
class ElementAnalysis:
    ...
    def full_result_plot(self, name: str):
        ...
        full_res: Result = dg(self.el, self.fl, self.tp, True, False)
        crop_res: Result = dg(self.el, self.fl, self.tp, True, True)
        ...
        fig = go.Figure(
            layout=dict(
                ...
                xaxis=dict(visible=True, title="Time (s)", range=[0, full_xvals[-1] if full_xvals is not None else len(self.measurement)]),
                yaxis=dict(title=f"Measurement ({crop_res.measurement.unit.replace("rad", "degrees")})"),
            ),
        )

        fig.add_trace(
            go.Scatter(x=full_xvals, y=u(full_res.measurement.value), name="Measurement", mode="lines", line=dict(color="blue", width=1, dash="dash"))
        )
        fig.add_trace(go.Scatter(x=crop_xvals, y=u(crop_res.measurement.value), name="Selected", mode="lines", line=dict(color="blue", width=1, dash="solid")))

        if len(dg.smoothers) > 1:
            fig.add_trace(go.Scatter(x=crop_xvals, y=u(crop_res.raw_sample), name="Visible Sample", mode="lines", line=dict(color="black", width=1, dash="dash")))
            fig.add_trace(go.Scatter(x=crop_xvals, y=u(crop_res.sample), name="Smooth Sample", mode="lines", line=dict(color="black", width=1, dash="solid")))
        else:
            fig.add_trace(go.Scatter(x=crop_xvals, y=u(crop_res.raw_sample), name="Sample", mode="lines", line=dict(color="black", width=1, dash="solid")))
        fig.add_trace(go.Scatter(
            x=crop_res.sample_keys[self.keys] if crop_xvals is None else crop_xvals[crop_res.keys],
        ...
        fig.add_vrect(x0=full_xvals[0],x1=crop_xvals[0],fillcolor="grey",opacity=0.2,line_width=0)
        fig.add_vrect(x0=crop_xvals[-1],x1=full_xvals[-1],fillcolor="grey",opacity=0.2,line_width=0)
