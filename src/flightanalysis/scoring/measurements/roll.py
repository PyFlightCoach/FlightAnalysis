...
@m.add
def roll_angle_gradient(fl: State, tp: State) -> Measurement:
    return Measurement(
        np.gradient(_roll_angle( fl, tp)) / fl.dt,
        "rad/s",
        *Measurement._roll_vis(fl, tp),
    )

def _roll_angle_proj(fl: State, tp: State, proj: g.Point) -> npt.NDArray:
...
@m.add
def roll_angle_p_gradient(fl: State, tp: State) -> Measurement:
    return Measurement(
        np.gradient(_roll_angle_proj( fl, tp, Measurement.get_axial_direction(tp))) / fl.dt,
        "rad/s",
        *Measurement._roll_vis(fl, tp),
    )
